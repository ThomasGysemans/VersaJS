...
/**
 * Checks if all the lines of the compiled code return `true`.
 * @param javascript The compiled JavaScript code.
 */
const check = (javascript: string) => {
    const javascript_lines = javascript.trim().split("\n");
    for (let i = 0; i < javascript_lines.length; ++i) {
        const evaluated = eval(javascript_lines[i]);
        if (!evaluated) {
...
const compile = (versa: string | string[]) => {
    ...
    transcriber.clearJavaScript() // remove the boilerplate
    transcriber.interpret_file();
    return transcriber.getJavaScript().trim();
};

before(() => {
    versacode = fs.readFileSync('./js/jscore.js', {encoding:'utf8'});
...
describe("JavaScript core", function() {
    ...
    it("should work with $.dictionary_equals", () => {
        assert.deepStrictEqual($.dictionary_equals({[`name`]: "thomas"}, {[`name`]: "thomas"}), true);
    });
    ...
    it("should work with an addition", () => {
        const versa = `
            ...
            # Every possible addition that returns a dictionary
