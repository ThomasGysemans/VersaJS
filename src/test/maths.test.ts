import type { BooleanValue, Value, ListValue } from '../values.js';
...
const check = (result: Value<any>) => {
    for (let i = 0; i < (result as ListValue).elements.length; ++i) {
        const bool = (result as ListValue).elements[i] as BooleanValue;
        if (typeof bool.state === "undefined") {
            console.error("line.state is undefined because it's:", bool);
        }
        if (bool.state === 0) {
            const error = new RuntimeError(
                bool.pos_start, bool.pos_end,
                "Expected true, but got false",
                context
            );
            console.error(error.toString());
        }
        assert.deepStrictEqual(bool.state, 1);
    }
};
