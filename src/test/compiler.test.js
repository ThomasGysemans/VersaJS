...
const evaluate  = (javascript, expected) => assert.deepStrictEqual(eval(javascript), expected);
const compare   = (javascript, expected) => assert.deepStrictEqual(javascript, expected);
const multiline = (javascript, expected) => {
    ...
    if (actual_lines.length !== expected_lines.length) {
        let problem = 0;
        let length = actual_lines.length > expected_lines.length ? actual_lines.length : expected_lines.length;
